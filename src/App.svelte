...
<main class="flex justify-center items-center h-screen bg-black text-white">
  ...
  <section class="text-center border-2 w-full mx-4 sm:w-1/2 py-4 space-y-4 rounded-sm">
    ...
    <div class="flex justify-center space-x-2">
      <input bind:value={newMessage} class="pl-2 text-black" type="text" placeholder="Change Message">
      <button on:click={() => {
        app.changeMessage(newMessage)
        newMessage = ""
      }} class="bg-green-400 px-2 hover:bg-green-500 transition-all">Submit</button>
    </div>
  </section> 
  {/if}
</main>